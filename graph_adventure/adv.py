...
def randomUnexploredDirection():
    # returns a random direction from the possible unexploredexits of the current room
    exits = player.currentRoom.getExits()
    unexploredExits = []

    # filter for unexplored
    for i in exits:
        if traversalGraph[player.currentRoom.id][i] == "?":
            unexploredExits.append(i)
    # print(unexploredExits)
    randomInt = random.randint(0, len(unexploredExits) - 1)
    return unexploredExits[randomInt]


def getOppositeDirection(direction):
    oppDirections = {"n": "s", "s": "n", "w": "e", "e": "w"}
...
def findShortestPathToUnexploredRoom(starting_vertex):
    q = Queue()
    ...
    while q.size() > 0:
        ...
        if vertex not in visited:
            visited.add(vertex)
            for next_vertex in traversalGraph[vertex].values():
                path_copy = list(path)
...
def traverseMaze():
    ...
    while len(traversalGraph) < len(roomGraph):
        ...
        if "?" in traversalGraph[player.currentRoom.id].values():
            roomA = player.currentRoom.id
            # get a random direction in the possible exits
            directionA = randomUnexploredDirection()

            # get the opposite direction
            oppDirection = getOppositeDirection(directionA)

            # travel in random direction
            player.travel(directionA)
            traversalPath.append(directionA)

            # log the direction in the last node and this node
            traversalGraph[roomA][directionA] = player.currentRoom.id

            # # populate node if not already populated
